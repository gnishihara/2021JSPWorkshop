...
(`readr`パッケージ )
* **`read_rds()`**: RオブジェクトをRDSファイルから読み込む。 (`readr` パッケージ)
* **`read_xlsx()`**: XLSXファイルをよむ。(`readxl` パッケージ)
...
* **`write_csv()`**: tibble をCSVファイルに書き込む。(`readr` パッケージ)
* **`write_rds()`**: RオブジェクトをRDSファイルに書き込む。 (`readr` パッケージ)

**日時操作の関数**

* **`parse_datetime()`**: 日時データの定義につかう。(`lubridate` パッケージ)
